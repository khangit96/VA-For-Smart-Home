...
def getImageFromStream():
    ...
    while True:   
        bytesImg += stream.read(1024)
        a = bytesImg.find(b'\xff\xd8')
        b = bytesImg.find(b'\xff\xd9')
        
        if a != -1 and b != -1:
            jpg = bytesImg[a:b+2]
            bytesImg = bytesImg[b+2:]
            global imgFromStream
            imgFromStream = cv2.imdecode(np.fromstring(jpg, dtype=np.uint8), cv2.IMREAD_COLOR)
            cv2.waitKey(5)
   ...
   # capture = cv2.VideoCapture(0)
    ...
    try:
        while True:
            ...
            if imgFromStream!='':
                rc = imgFromStream
                fullSizeBaseImage= imgFromStream
...
if __name__ == '__main__':
    thread = threading.Thread(target=getImageFromStream)
    thread.start()
